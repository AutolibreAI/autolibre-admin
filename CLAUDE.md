...
| `/leads/pedidos` | `true` | El `listar-pedidos-de-presupuesto-abiertos.sql` que el backend tenía en `scripts/sql/` y borró el 2026-09-15 (`… where status <> 'closed' order by created_at`) más los cerrados que nadie mira: los `QuoteRequest` (NO `Lead`) con estado, canal, contacto, cuenta, patente vs vehículo vinculado, tiempos a contacto/respuesta, resultado del operador y del usuario. **La lista es read-only (escribe la ficha), y `quote_requests` NO existe en producción al 2026-09-14**: un guard (`quoteRequestsAvailability`) lo detecta y la pestaña dice "no desplegado" en vez de un 500. → `.claude/rules/leads.md` |
| `/leads/pedidos/:id` | `true` | El `select * from quote_requests where id = '…'` de antes de llamar, con el recorrido ordenado por timestamp, las `internal_notes` como hilo (sello en hora de Buenos Aires) y el `raw_submission`. **Escribe**: marcar contactado / respondido, cerrar y agregar nota interna, por los SPs de `ops` de la migración 011 con el actor de la sesión (`cancelled_by_user` no se ofrece: es de la app). Sin tabla no hay botones, y cada escritura re-chequea la disponibilidad en el handler |
...
2. **`ops.schema_migrations` es por base.** Aplicar en DEV no aplica en PROD. Una migración nueva se
   ...
   **Al 2026-09-15 la 011 (transiciones de pedidos de presupuesto) está escrita y probada, y NO
   aplicada en ninguna base** — su suite corrió con la migración adentro del `ROLLBACK`.
