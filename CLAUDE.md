...
| `/leads/pedidos/:id` | `true` | El `select * from quote_requests where id = '…'` de antes de llamar, con el recorrido ordenado por timestamp, las `internal_notes` como hilo (sello en hora de Buenos Aires) y el `raw_submission`. **Escribe**: marcar contactado / respondido, cerrar y agregar nota interna, por los SPs de `ops` de la migración 011 con el actor de la sesión (`cancelled_by_user` no se ofrece: es de la app). Sin tabla no hay botones, y cada escritura re-chequea la disponibilidad en el handler. Suma plantillas de mensajes completadas con los datos del pedido (`QuoteTemplates`, no escriben nada) y el link a WhatsApp |
