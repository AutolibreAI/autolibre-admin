...
| `/metricas` | `'data-only'` | 2º del menú, pegada a Inicio. Antes era `/graficos`. Cinco bloques: (1) las 4 cards del pulso de Inicio, reusadas vía `PulseRow`; (2) **la consulta que nadie corría** — qué % de los usuarios reales usó cada función (seguro, multas, chat IA, VTV, escáner, mantenimiento…), una docena de `count(distinct user_id)` en una sola sentencia; (3) Crecimiento: el alta de `users`/`vehicles` período a período (agrupado en hora de Buenos Aires desde el 2026-09-23, no UTC) + **altas con vehículo en el mismo proceso** (`onboardingSeries`, corte de 10 min) + la distribución de autos por usuario; (4) **Pedidos** — cuatro series sobre `quote_requests` + `ops.quote_request_response` (recibidos, propuestas por pedido, tiempo hasta contactado/respondido, red vs afuera), mismo corte de duplicados que la card de Inicio. No confundir con el `/analytics` borrado el 2026-08-30 (renderizaba `RecordItem` placeholder). → `.claude/rules/metricas.md` |
