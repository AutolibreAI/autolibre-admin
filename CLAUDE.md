...
| `/leads/pedidos` | `true` | `scripts/sql/listar-pedidos-de-presupuesto-abiertos.sql` del backend (`… where status <> 'closed' order by created_at`, aunque el default de esta pantalla ordena `createdAt desc` desde el 2026-09-15) más los cerrados que nadie mira: los `QuoteRequest` (NO `Lead`) con estado, canal, contacto, cuenta, patente vs vehículo vinculado, tiempos a contacto/respuesta, resultado del operador y del usuario. **Escribe el estado** desde el 2026-09-15 (`ops.advance_quote_request`, migración 011) y **carga pedidos informales a mano** (`ops.create_quote_request`, migración 012, botón "Cargar pedido") — agregar una nota interna sigue siendo el script SQL del backend. `quote_requests` no existía en producción al 2026-09-14 y existe desde antes del 2026-09-15; un guard (`quoteRequestsAvailability`) sigue protegiendo contra una base que todavía no migró. → `.claude/rules/leads.md` |
| `/leads/pedidos/:id` | `true` | El `select * from quote_requests where id = '…'` de antes de llamar, con el recorrido ordenado por timestamp, las `internal_notes` como hilo (sello en hora de Buenos Aires) y el `raw_submission`. **El estado se cambia desde acá** (mismo `<QuoteStatusControl/>` que el listado); agregar una nota interna sigue siendo un script SQL del backend |
