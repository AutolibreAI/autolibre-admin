...
| `/graficos` | `'data-only'` | Nada previo: el crecimiento de `users` y `vehicles` en el tiempo no se graficaba (Inicio da el snapshot, no la curva). Serie de altas por período + acumulado, con la unidad temporal a elección (día/semana/mes/año). No confundir con el `/analytics` borrado el 2026-08-30 — ése renderizaba `RecordItem` placeholder; éste es `date_trunc` real sobre `created_at` |
