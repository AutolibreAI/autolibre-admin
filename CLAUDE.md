...
| `/leads/pedidos/:id` | `true` | El `select * from quote_requests where id = '…'` de antes de llamar, con el recorrido ordenado por timestamp, las `internal_notes` como hilo (sello en hora de Buenos Aires) y el `raw_submission`. Las transiciones siguen siendo los 4 scripts SQL del backend |
