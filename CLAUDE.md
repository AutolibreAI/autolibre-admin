...
| `metricas.md` | La pantalla `/metricas` (antes `/graficos`): `PulseRow` compartido con Inicio, la tabla de adopción por función (una sola sentencia, "usuario real" reusado de `INTERNAL_PREDICATE`, VTV filtra `file_id IS NOT NULL`, orden fijo por % sin `SortHeader`), y que no hay ninguna escritura |
