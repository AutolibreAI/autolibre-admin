...
| `/actividad` | `true` | **La consulta que no se corría porque no se puede**: qué está pasando en la app, ahora. Cada pantalla del panel mira UNA tabla, así que esto son quince `select … order by created_at desc limit 20` sueltos y un merge a ojo por timestamp. Es un `UNION ALL` de 16 ramas —todo lo que sólo existe porque una persona tocó la app: alta, vehículo, chat, escaneo, los 4 documentos, mantenimiento, plan, pedido, consulta de multas, consulta de patente, dispositivo, login, feedback— con quién, qué, sobre qué auto y con qué resultado. Lo automático NO entra (las notificaciones son `/notificaciones`; de las multas entra la consulta, no las 659 filas del proveedor). Ni una escritura. → `.claude/rules/activity-feed.md` |
...
| `/notificaciones/reglas` | `true` | Dos secciones. **Recordatorios de vencimiento**: el `select * from notification_rules` + el `count(*) … group by rule_id` contra `notifications` que nadie corría — las 22 reglas del backend (seguro, VTV, licencia, mantenimiento × días/km antes) agrupadas por documento, con avisos generados, último envío y el texto real del último aviso. **Sólo lectura**: el texto vive en el código del backend, y pausar/agregar espera la respuesta del backend (si su seed reactiva reglas en cada deploy) → `.claude/plans/reglas-de-vencimiento.md`. **Envíos programados**: las reglas del panel (`ops.notification_schedule`, migración 014), con alta, edición, pausa y baja; nacen pausadas y todavía no hay motor que las dispare. → `.claude/rules/notifications.md` |
| `/notificaciones/reglas/:id` | `true` | La ficha de un envío programado (`ops.notification_schedule`): edición completa, pausar/reanudar, borrar si no corrió nunca |
