...
| `/actividad/:tipo/:id` | `true` | El `select * from <tabla> where id = '…'` más los joins de usuario y vehículo, para los OCHO tipos que todavía no tienen pantalla dueña (vehículo, mantenimiento, plan, las dos consultas, dispositivo, login, feedback). Los otros ocho no pasan por acá: la fila los manda directo a `/chats/:id`, `/escaneres/sesiones/:id`, `/documentos/:tipo/:id`, `/leads/pedidos/:id` o `/usuarios/:id`, que muestran mucho más. El destino es un `<Link>` tipado por tipo, y el `default` no compila si aparece un tipo sin destino |
| `/solicitudes` | `true` | Las consultas 1–6 del runbook `aprobar-partner-application.sql` (borrado del backend el 2026-09-15; ver `partner-approval.md`). La ficha de una solicitud (`/solicitudes/$applicationId`) además EDITA todos sus campos vía `ops.update_partner_application` (migración 010) — `status` sigue aparte |
