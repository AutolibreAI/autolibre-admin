...
| `/vehiculos` | `true` | La docena de `select` sueltos por `vehicle_id` que hoy hacen falta para saber "¿qué tan sano está este auto?" — contra `driving_sessions`, `conversations`, `maintenance_occurrences`, `notifications`, `fines`, `insurances`, `vehicle_inspections`, `registration_cards` y `diagnostic_dtcs`. Es `/usuarios` invertido: el usuario es una columna, el auto es la fila |
| `/vehiculos/:id` | `true` | Ídem, entero: el censo de 21 relaciones del vehículo, y la pieza que no existía en ningún lado — separar los DTC del último escaneo ("activos") de los que aparecieron antes y ya no están ("inactivos") |
...
| `vehicles.md` | La ficha del vehículo: por qué el censo es de 21 relaciones, cómo se separan los DTC "activos" de los "inactivos" con el caso real que lo prueba, por qué "alertas activas" es una lectura del panel y no una columna, y por qué acá `join` (no `left join`) contra el catálogo es el mismo caso que `findUserDetail`, no una regla nueva |
