...
| `/metricas` | `'data-only'` | 2º del menú, pegada a Inicio. Antes era `/graficos`. Seis bloques: (1) las 4 cards del pulso de Inicio, reusadas vía `PulseRow`; (2) **la consulta que nadie corría** — qué % de los usuarios reales usó cada función (seguro, multas, chat IA, VTV, escáner, mantenimiento…), una docena de `count(distinct user_id)` en una sola sentencia; (3) Crecimiento: el alta de `users`/`vehicles` período a período (agrupado en hora de Buenos Aires desde el 2026-09-23, no UTC) + **altas con vehículo en el mismo proceso** (`onboardingSeries`, corte de 10 min) + la distribución de autos por usuario; (4) **Pedidos** — cuatro series sobre `quote_requests` + `ops.quote_request_response` (recibidos, propuestas por pedido, tiempo hasta contactado/respondido, red vs afuera), mismo corte de duplicados que la card de Inicio; (5) desde el 2026-09-24, **dónde están radicados** los autos activos: AMBA (CABA + los 40 municipios de la Región Metropolitana) / resto de PBA / interior por provincia, leído de `vehicle_plate_lookups` (→ `.claude/rules/vehicle-location.md`). No confundir con el `/analytics` borrado el 2026-08-30 (renderizaba `RecordItem` placeholder). → `.claude/rules/metricas.md` |
| `/actividad` | `true` | **La consulta que no se corría porque no se puede**: qué está pasando en la app, ahora. Cada pantalla del panel mira UNA tabla, así que esto son quince `select … order by created_at desc limit 20` sueltos y un merge a ojo por timestamp. Es un `UNION ALL` de 16 ramas —todo lo que sólo existe porque una persona tocó la app: alta, vehículo, chat, escaneo, los 4 documentos, mantenimiento, plan, pedido, consulta de multas, consulta de patente, dispositivo, login, feedback— con quién, qué, sobre qué auto y con qué resultado. Desde el 2026-09-25 tampoco entran los chats sin mensajes ni los pedidos cerrados como duplicado (duplicados y pruebas del equipo). Lo automático NO entra (las notificaciones son `/notificaciones`; de las multas entra la consulta, no las 659 filas del proveedor). Ni una escritura. → `.claude/rules/activity-feed.md` |
| `/actividad/:tipo/:id` | `true` | El `select * from <tabla> where id = '…'` más los joins de usuario y vehículo, para los SIETE tipos que todavía no tienen pantalla dueña (vehículo, mantenimiento, plan, las dos consultas, dispositivo, login). Los otros nueve no pasan por acá: la fila los manda directo a `/chats/:id`, `/escaneres/sesiones/:id`, `/documentos/:tipo/:id`, `/leads/pedidos/:id`, `/feedback` o `/usuarios/:id`, que muestran mucho más. El destino es un `<Link>` tipado por tipo, y el `default` no compila si aparece un tipo sin destino |
| `/solicitudes` | `true` | Las consultas 1–6 del runbook `aprobar-partner-application.sql` (borrado del backend el 2026-09-15; ver `partner-approval.md`). La ficha de una solicitud (`/solicitudes/$applicationId`) además EDITA todos sus campos vía `ops.update_partner_application` (migración 010) — `status` sigue aparte. Desde el 2026-09-23 la vista de lectura también muestra el "otros" de rubros y de "cómo nos conoció" (antes sólo se veían adentro del formulario de edición), y el listado marca "declaró otros" |
...
| `/partners/$partnerId` | — | La ficha: carga manual de rubros (consultas 7–8 del runbook) + editor de estado, coordenadas, contacto, perfil y links. Desde el 2026-09-23, al lado del editor de rubros hay una tarjeta de sólo lectura con lo que el taller declaró al registrarse (rubros, "otros", marcas, combustibles) y el link a su solicitud — "vino de la planilla" cuando no hay `applicationId` |
...
| `/leads/pedidos` | `true` | El `listar-pedidos-de-presupuesto-abiertos.sql` que el backend tenía en `scripts/sql/` y borró el 2026-09-15 (`… where status <> 'closed' order by created_at`, aunque el default de esta pantalla ordena `createdAt desc` desde el 2026-09-15) más los cerrados que nadie mira: los `QuoteRequest` (NO `Lead`) con estado, canal, contacto, cuenta, patente vs vehículo vinculado, tiempos a contacto/respuesta, resultado del operador y del usuario. **La fila no mueve estados (eso es la ficha)**; lo único que escribe es **cargar un pedido informal a mano** (`ops.create_quote_request`, migración 012, botón "Cargar pedido"). `quote_requests` no existía en producción al 2026-09-14 y existe desde el 2026-09-15; el guard (`quoteRequestsAvailability`) sigue protegiendo contra una base que todavía no migró. → `.claude/rules/leads.md`. Desde el 2026-09-25 el resumen y el listado NO cuentan los cerrados como `duplicate` — el equipo los usa también para descartar sus pedidos de prueba; el listado los trae con el chip "Incluir duplicados / prueba" |
| `/leads/pedidos/:id` | `true` | El `select * from quote_requests where id = '…'` de antes de llamar, con el recorrido ordenado por timestamp, las `internal_notes` como hilo (sello en hora de Buenos Aires) y el `raw_submission`. **Escribe**: marcar contactado / respondido, cerrar y agregar nota interna, por los SPs de `ops` de la migración 011 con el actor de la sesión (`cancelled_by_user` no se ofrece: es de la app). Sin tabla no hay botones, y cada escritura re-chequea la disponibilidad en el handler. Suma plantillas de mensajes completadas con los datos del pedido (`QuoteTemplates`, no escriben nada) y el link a WhatsApp. Desde el 2026-09-22 **también carga lo que contestó cada taller** (`QuoteResponses`, migración 015, tabla `ops.quote_request_response`): con precio, con precio cero («sin cargo») o sin precio, con la dirección y el teléfono del taller de afuera del directorio. Con eso la plantilla «Presupuestos» arma SOLA el mensaje de WhatsApp —numerado, con 📍📞🕘💵— y el botón lo manda con el texto ya editado. Desde el 2026-09-23 el `AL-xxxx` va en las CUATRO plantillas (guardrail en tiempo de módulo) y hay dos más para pedirle cotización a un TALLER (`cotizacion_red`, `cotizacion_nuevo`) — con menos dato que las de la persona (sin nombre, teléfono ni patente: es la deuda de Ley 25.326 que el backend marcó bloqueante), y un botón "Pedir cotización" por candidato en `PartnerCandidates` que abre WhatsApp con la plantilla «red» ya renderizada. Revisado el 2026-09-25: las plantillas dejaron de ser una constante fija — son versiones editables en `ops.quote_message_template_version` (migración 017, la semilla de código queda como default hasta la primera edición), con un editor en `QuoteTemplates` (título, audiencia, variables, vista previa, historial con "usar esta versión"). Los rubros pasaron de uno a VARIOS por pedido (`ops.set_quote_request_rubros`, migración 016, reemplaza a la 013), y `PartnerCandidates` ordena a los candidatos por cuántos rubros pedidos cubren. El cierre agrega la nota al hilo en la MISMA operación (`p_internal_note`), y las notas de auditoría/cierre/resultado/presupuesto sueltas se sacaron del formulario — sólo queda el hilo. Suma una tarjeta de sólo lectura del vehículo vinculado (VIN, motor con su fuente, km, DTCs del último escaneo, tareas) |
| `/leads/contactos` · `/leads/financiacion` | — | **Nada todavía.** Pestañas "próximamente": la excepción relevada el 2026-09-06 a "una pantalla que no reemplaza una consulta no va". Se decidió mostrarlas como plan visible. → `.claude/rules/leads.md` |
...
| `/vehiculos` | — | Layout de 2 pestañas (era de 3 hasta el 2026-09-17: Catálogo y "Flota" se fusionaron). Antes era `/catalogo`. Redirige a `/vehiculos/catalogo`. → `.claude/rules/vehicles.md` |
| `/vehiculos/catalogo` | `true` | Los 210 modelos del catálogo con su flota, unificados: nada previo (`vehicle_catalog_manuals` tenía CERO filas contra ~80 catálogos, y el `INSERT` que hacía falta era **imposible** a mano — `file_id` referencia una fila de `files` que sólo existe si el PDF se subió a DigitalOcean Spaces) más la consulta que nadie corría, "¿cuáles son los modelos más comunes de la flota, y cómo se comporta cada uno?" (usuarios, km promedio, con multas, deuda total, escaneados). Universo completo: incluye los 30 modelos —de 210, al 2026-09-17— sin un solo auto, que antes la vieja pestaña "Flota" escondía y que son un pendiente real, no un resto (`vehicles` apunta al spec, no al catálogo, así que un modelo sin variante no puede tener auto). Cada fila se despliega (desde el 2026-09-23) y muestra QUIÉN tiene ese modelo — el espejo del toggle de `/usuarios`, al click, no en el loader → `.claude/rules/vehicles.md` |
...
| `/vehiculos/listado` | `true` | El padrón entero de autos, uno por fila, sin deduplicar por patente, con lo que cuelga de cada uno (VTV, seguro, multas, deuda, tareas, escaneos). Es `UserVehicleSummary` de `/usuarios` pero transversal y con dueño. Desde el 2026-09-24 suma **la radicación** (columna + filtros por región y provincia): el cruce `vehicles` → `vehicle_plate_lookups` por patente, con la localidad de PBA pasada a partido para saber si es AMBA — que nadie armaba. → `.claude/rules/vehicle-location.md` |
| `/escaneres` | — | Layout de 4 pestañas. Redirige a `/escaneres/compatibilidad`. → `.claude/rules/scanner-compatibility.md` · `.claude/rules/scan-sessions.md` · `.claude/rules/scan-detections.md` · `.claude/rules/scan-compare.md` |
| `/escaneres/compatibilidad` | `true` | **La consulta que no se corría porque no se te ocurre**: con qué versión de auto —`TOYOTA COROLLA XEI 1.8 M/T 2013`, no "un Corolla"— funcionó cada escáner y con cuáles no. Cruza `driving_sessions` → `vehicles` → `vehicle_catalog_specs` → `vehicle_catalogs`, y separa los intentos que trajeron datos de los que engancharon y no trajeron nada. Cada celda abre el historial de esas conexiones (usuario, firmware, protocolo, VIN detectado, DTCs, qué produjo). Lo que sí pasaba: contestar "¿le recomiendo este escáner a un Vento?" de memoria |
| `/escaneres/sesiones` | `true` | El `select * from driving_sessions order by started_at desc` que nadie corre, más los `select count(*)` sueltos por `session_id`: cada escaneo en una fila con usuario, fecha, duración, DTCs (cuántos y cuáles), anomalías (cuántas, cuáles y de qué severidad), distancia desde el borrado de fallas, VIN detectado, batería, firmware y métricas de manejo. Ordenable y filtrable por columna. Desde el 2026-09-25 la fila ENTERA abre la ficha, y la ficha muestra cada PID como gráfico de rango con etiqueta y unidad, los DTC con título, la historia del auto y el último mantenimiento previo → `.claude/rules/scan-sessions.md` |
| `/escaneres/detecciones` | `true` | **La consulta que nadie arma**: cada código DTC y cada tipo de anomalía en una fila, con cuántas sesiones lo vieron (y %), en cuántos vehículos únicos (y %), en cuántos modelos del catálogo, desde/hasta cuándo, y el título del DTC leído de `src/server/dtc-codes.json`. Unnestea `session_dtc_snapshots.codes` (universo: sesiones `completed`) y el jsonb `driving_telemetry_analysis.anomalies` (universo: + con datos) en un `UNION ALL`. Arriba, un bloque con los DTC detectados que faltan cargar en el JSON. Vista transversal de lo que `/escaneres/sesiones` muestra fila por fila |
...
| `/notificaciones` | `true` | Layout de 2 pestañas. **NO redirige**: el índice ES el historial, porque `/usuarios/:id` y `/operacion` ya linkeaban a la raíz con search params. → `.claude/rules/notifications.md` |
| `/notificaciones` (Historial) | `true` | El `select * from notifications where user_id = '…'` que hoy es la única forma de ver de qué le avisamos a alguien y si le llegó — con lo que ese select no contesta solo: el estado de entrega leído de `status` + `delivery_status` juntos (`sin_token` reintenta para siempre, `rechazada` es terminal), y `atrasada` deducido del reloj con el MISMO umbral que el `stuck` de `/operacion`. Relevado el 2026-09-07: 35 de 72 filas estaban atrasadas. **Escribe**: "Nueva notificación" (`BroadcastComposer`) manda un push ad-hoc a usuarios elegidos — reemplaza el `curl` a `POST /notifications/broadcast` con ids pegados a mano. Va por HTTP al backend, no por SQL; `?notificationBroadcastId=` filtra las filas de un envío. Desde el 2026-09-16 los destinatarios se pueden **armar por condición** (`AudienceBuilder`): reemplaza el `select u.id from users u where <condición>` de DBeaver y pegar uuid de a uno — lo que viaja es `{field, op, value}` de enums cerrados, nunca SQL, y resuelve a ids explícitos ANTES de enviar |
| `/notificaciones/envios` | `true` | **La consulta que nadie corría**: el `group by source_id` que agrupa las filas de un envío ad-hoc. Hasta el 2026-09-16 el resultado de un envío se veía UNA vez, en el compositor, y se perdía al recargar |
| `/notificaciones/envios/:id` | `true` | Un envío: quién lo recibió, en qué estado quedó cada fila, y **qué hizo cada persona después** — seis acciones (cargó auto, documento, escaneo, chat, dispositivo, mantenimiento) medidas contra el momento de SU notificación, con el denominador honesto ("de los que NO lo habían hecho"). Es correlación y lo dice. Sin tabla de campañas: no se puede saber con qué condición se eligió la audiencia, ni cuánto tardaron en leerla (`notifications` no tiene `read_at`) |
| `/feedback` | `true` | El `select * from feedback order by submitted_at desc` + el join a `users` que hoy nadie corre — antes el feedback sólo se veía como una fila de `/actividad` o un contador en la ficha del usuario. Mensaje COMPLETO, sin truncar; filtros por plataforma y versión de app (data-driven); contexto por fila (autos, última actividad, cuántos feedbacks mandó esa persona). **Escribe** por HTTP: "Responder" abre `BroadcastComposer` con ese usuario ya cargado como destinatario, mismo `POST /notifications/broadcast` que `/notificaciones`. → `.claude/rules/feedback.md` |
...
`approve_partner_application()`, la vista `v_partner_application_queue` y los runbooks que vivían en
`autolibre-backend-hex/scripts/sql/` —hoy SPs de `ops` o pantallas de este repo— no son un detalle de implementación que convenga esconder detrás
...
2. **`ops.schema_migrations` es por base.** Aplicar en DEV no aplica en PROD. Una migración nueva se
   ...
   **Al 2026-09-15 la 011 (transiciones de pedidos de presupuesto) está aplicada en DEV y NO
   verificada en producción**: la aplica el `vercel-build` del próximo deploy de `main`. Es la
   única 011: la de `origin/config-pedidos` (`advance_quote_request`) se descartó en el merge
   → `.claude/rules/ops-write-actions.md`. **La 012 (`ops.create_quote_request`, cargar pedidos
   a mano) vino de esa rama en el segundo merge y NO está aplicada en DEV**: `pnpm db:migrate`
   antes de probar "Cargar pedido".
...
| `activity-feed.md` | El feed `/actividad`: el corte "lo que hizo una persona" (y por qué quedan afuera las notificaciones, las multas del proveedor, la VTV consultada y las aceptaciones legales), el alias obligatorio en las 16 ramas del `UNION ALL`, el filtro de tipo que PODA en vez de filtrar, por qué NO hay `limit` por rama (con `q` afuera daría un resultado incorrecto), `age_minutes` calculado por Postgres para no romper la hidratación, el `<Link>` tipado cuyo `default` no compila si falta un destino, el id de `consulta_multas` que es el del vehículo, y la diferencia con `~/lib/activity` (las señales de churn) |
| `metricas.md` | La pantalla `/metricas` (antes `/graficos`): `PulseRow` compartido con Inicio, la tabla de adopción por función (una sola sentencia, "usuario real" reusado de `INTERNAL_PREDICATE`, VTV filtra `file_id IS NOT NULL`, orden fijo por % sin `SortHeader`), que Crecimiento agrupa en hora de Buenos Aires (a diferencia del resto del panel), las altas con vehículo en el mismo proceso (`onboardingSeries`, corte de 10 min), la sección Pedidos (`quoteRequestSeries`, el guard de dos niveles, por qué 2b/2d salen de `ops.quote_request_response` y no de `proposals_count`, y por qué "tiempo hasta respondido" es aproximado), y que no hay ninguna escritura |
| `ops-write-actions.md` | Los SP de `ops` que escriben `public`: por qué se permiten, los 8 guardrails, las dos minas, los dos SP que se decidió NO escribir, y la 011 (pedidos de presupuesto: la auditoría sin la ubicación exacta y los parámetros `text` porque la tabla no existe en producción; con botones en `/leads/pedidos/:id` e integración por `PREPARE` sin tipos). Suma la **015** (lo que contestó cada taller: por qué es una tabla de `ops` AUNQUE el backend tenga `quote_request_proposals` —no admite precio nulo ni el contacto de un taller de afuera, que son la mitad de los casos— y el `INSERT … SELECT` del backfill para el día que sí; el guardrail 8 invertido porque en `ops` no hay trigger; el precio con TRES estados y el CHECK en `>= 0`; el helper único compartido por alta y edición; el reorden con la lista completa y una sola entrada de log). Suma la **016** (rubros múltiples — `ops.quote_request_rubro` se DROPEA y recrea con PK compuesta, `set_quote_request_rubro` singular se retira por `set_quote_request_rubros`; y `p_internal_note` atómico con el cierre, DROP + CREATE otra vez la trampa de la 009) y la **017** (plantillas de mensaje versionadas, append-only, `before` con contenido real por primera vez en vez de `NULL`, `archived` con SP listo y sin botón todavía) |
...
| `leads.md` | Las pestañas de `/leads`: por qué "sección" acá es vista de producto y no el `Lead` del backend, qué pestaña tiene datos y por qué las otras dos son "próximamente", por qué Contactos no se puede arrancar desde este repo, las trampas de Seguros (días del vencimiento y no de `status`, `JOIN` y no `LEFT`, aseguradora cruda, PDF no descargable) y las de Multas (grano = vehículo consultado, `$0` ≠ null, predicado `pending` compartido con `users.repo`, `ORDER BY` desde `Record` cerrado, "desactualizada" como lectura del reloj) y las de Pedidos (`QuoteRequest` ≠ `Lead`, una sola tabla sin presupuestos por taller, el guard de disponibilidad porque `quote_requests` no existe en producción, `outcome` vs `user_outcome` y `NULL` ≠ `no_response`, `closed` incluye cancelaciones del usuario, notas en hora de Buenos Aires, vehículo vinculado de otra cuenta, deuda de Ley 25.326, y las transiciones del operador como SPs de `ops` de la 011 con botones en la ficha: guard también antes de cada escritura, `p_note` de auditoría ≠ nota interna, nota de un solo renglón, error en el componente que no se desmonta) y los **presupuestos** (la premisa "no hay presupuestos por taller" quedó vieja el 2026-09-22 y está corregida ahí, DOS veces: existen, y NO van en la tabla del backend; el precio tiene tres estados —sin precio, sin cargo, con precio—, el taller es partner O nombre libre con su contacto, el orden es editorial y no por precio, una vigencia vencida no entra en el mensaje, `proposals_count` es otro número y no se sincroniza, y el PDF queda afuera). Suma el **mensaje de WhatsApp** armado desde esas filas, con las cuatro decisiones que salieron de un mensaje real del operador |
| `notifications.md` | Notificaciones: por qué `status` y `delivery_status` son dos ejes y cómo se combinan en un estado derivado, `sin_token` vs `rechazada` (reintenta para siempre vs terminal), el umbral de `atrasada` compartido con `ops.repo.ts`, por qué los search params van calificados (`notificationType`/`notificationState`/`notificationBroadcastId`), y la única escritura: el envío ad-hoc por HTTP a `POST /notifications/broadcast` (idempotencia por `broadcastId`, 204 sin cuenta, un id malo tira el lote). Suma las **audiencias por condición** (catálogo cerrado en vez de SQL libre, Y sin O, `null` no matchea, resolución a ids explícitos, el corte estable de 500 con `count(*) over ()`) y los **envíos** (un envío es un `group by source_id`, `DERIVED_STATE` exportado, las seis acciones de "qué pasó después" con el denominador "no lo había hecho antes", y las dos cosas que NO se pueden medir: la audiencia usada y el `read_at` que no existe) |
...
| `vehicle-location.md` | La radicación de vehículos: de dónde sale (`vehicle_plate_lookups`, domicilio del TITULAR, no del usuario), AMBA = los 40 municipios (decisión de producto del 2026-09-24, una sola constante), el clasificador de tres niveles (provincia normalizada, localidad → partido con el dataset de Georef, región), la regla de ambigüedad (nunca elegir partido por orden), los alias, "SQL agrupa por el crudo, JS clasifica" con la tabla inyectada por `unnest` (`locationJoin`), dónde se ve, los cuadres, y la trampa de los escapes `\uXXXX` que la herramienta decodifica |
...
| `scan-compare.md` | El comparador `/escaneres/comparar`: qué dato hay (el resumen por PID, no las curvas — DigitalOcean fuera), los tres niveles de "similar" sin adivinar cilindrada, el universo (JOIN al análisis y al catálogo), la selección en el cliente sobre un snapshot, el valor típico por escaneo Y por auto con el piso de `MIN_VEHICLES_FOR_CONFIDENCE`, por qué se AGRUPA en vez de pintar (no hay paleta categórica que pase), las lecturas dudosas de `~/lib/scan-pids`, y `PidRangeChart` compartido con la ficha |
